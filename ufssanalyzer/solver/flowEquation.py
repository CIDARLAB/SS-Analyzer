...
class FlowEquation:
    def __init__(self):
        self.unknown: str = ""  # This is the CPoint we are solving for
        self.multiplier = 0
    ...
    @staticmethod
    def constructFlowEquation(enetwork: ENetwork, cpoint: CPoint) -> FlowEquation:
    ...
    @staticmethod
    def constructType1Equation(enetwork: ENetwork, cpoint: CPoint) -> FlowEquation:
        ret = FlowEquation()
        ret.unknown = cpoint.id

        # Add the flow rate to the constant term
        assert cpoint.flowrate is not None
    ...
    @staticmethod
    def constructType2Equation(enetwork: ENetwork, cpoint: CPoint) -> FlowEquation:
        ret = FlowEquation()
        ret.unknown = cpoint.id
    ...
    @staticmethod
    def constructType3Equation(enetwork: ENetwork, cpoint: CPoint) -> FlowEquation:
        ret = FlowEquation()
        ret.unknown = cpoint.id
        # go through the neighbours
        neighbors = enetwork.G.neighbors(cpoint.id)
